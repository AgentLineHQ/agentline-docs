...
- `call.received` — inbound answer
- `call.utterance` — a live caller turn. Its `conversation` field uses `user` and `assistant`.
- `call.completed`
- `call.owner_task`
- `call.failed`
- `call.busy`
...
`GET /v1/calls/{id}/transcript` uses different role names: `human` and `agent`.
