...
The backend publishes these `event_type`s:

- `call.received` — an inbound call started
- `call.utterance` — a live caller turn
- `call.completed` — the call ended with status `completed`
- `call.failed` — the call ended with status `failed`
- `call.busy`, `call.no-answer`, `call.canceled` — other terminal provider statuses
- `call.owner_task` — a completed call with the owner
- `sms.received` — an inbound SMS
