...
```json
{"type":"context","event_id":"evt_xxx","call_id":"call_xxx","turn_id":"turn_xxx","push_token":"...","context":"No new emails.","disposition":"done"}
...
`context` is short facts, not a script. The hosted voice does not speak that
text aloud.
...
curl -X POST "https://api.agentline.cloud/v1/calls/call_xxx/context?turn_id=turn_xxx" \
  ...
  -d '{"turn_id":"turn_xxx","context":"No new emails.","disposition":"done"}'
```
