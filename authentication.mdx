...
description: Every request uses a Bearer API key (al_live_...).
---
...
<CodeGroup>
  ...
  ```
  ```javascript
  const res = await fetch("https://api.agentline.cloud/v1/agents", {
...
MCP clients use the same Bearer key against
`https://api.agentline.cloud/mcp` via `mcp-remote`. See [MCP](/guides/mcp).
Outbound SMS is not an MCP tool.
